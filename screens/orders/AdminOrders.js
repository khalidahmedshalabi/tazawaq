...
import { Dimensions, KeyboardAvoidingView, AsyncStorage,
        ...
        Platform, DeviceEventEmitter } from "react-native";
...
export default class Signin extends React.Component {
    ...
    listeners = {
        update: DeviceEventEmitter.addListener('ReloadStoreOrders', ({ empty }) => {
            AsyncStorage.getItem('owner_login').then(
                (logged) => {
                    ...
                    if(logged == '1')
                    {
                        AsyncStorage.getItem('storeid').then(
                            (val) => {
                                if(val === null) return;
    ...
    fetchStoreOrders = (storeid) => {
        fetch(`${Server.dest}/api/get-store-orders?store_id=${storeid}`, {headers: {'Cache-Control': 'no-cache'}}).
    ...
    loginOwner = () => {
        ...
        fetch(Server.dest + '/api/store-owner-login?passname='+this.state.passname+
            '&password='+this.state.password, {headers: {'Cache-Control': 'no-cache'}}).
        then((res) => res.json()).then((resJson) => {
            console.log(resJson.response);
            if(resJson.response == 0)
                this.setState({ errorMsg: 'هذا المحل التجاري غير مُسجل عندنا او كلمة مرور غير صحيحة'});
            else
            {
        ...
        else
        {
            return (
                <View style={{ height: '100%' }}>
                    <Table style={styles.table} borderStyle={{borderWidth: 0.5, borderColor: Colors.mainColor }}>
                        <Row data={["بيانات الزبون", "الموقع", "السعر", "التفاصيل", "الحالة"]} style={styles.head} textStyle={styles.headText} flexArr={[1, 1, 1, 2, 1]}/>
                        <Rows data={this.state.orders} style={styles.row} textStyle={styles.text} flexArr={[1, 1, 1, 2, 1]}/>
                    </Table>
               </View>
            );
        }
...
const styles = StyleSheet.create({
    ...
    text: { paddingVertical:10, textAlign: 'center', color: Colors.secondaryColor },
